...
/*
 ...
 ----------------- 28/11/2012 --------------------
 cambio funcion deprecated "ereg" en linea 978 por "preg_match"
...
class ModbusTcp {
	...
	function SetSimulation() {
		...
		echo "<br><font color='#090762' size=3><b>Atención valores en modo simulación ! ! ! ! </b></font><br>";
	}
	...
	function BytesToFloat( $byte1, $byte2, $byte3, $byte4 ) {
		...
		$src = ( ($byte3 & 0x000000FF) << 24) + (($byte4 & 0x000000FF) << 16) + (($byte1 & 0x000000FF) << 8) + (($byte2 & 0x000000FF) );
	...
	function print_r_log($var) { 
		echo "<pre><font color='#090762' size='1' face='Verdana'>";
		print_r($var);
		echo "</pre><br>";
	}
...
//				echo "debut-adresse = $this->DebutAdresse <br>";
...
//				$buffer[400000 + $this->DebutAdresse + $i] = ( 256*rand(65, 90) + rand(65, 90) ) ;//Simulation ASCII
	...
	function ReadDiscretInputs() {
		...
		if ( $this->Debug ) { //Affichage des octets emis si en mode Debug
			echo "<b>Lectura entradas discretas</b><br>"; 
			for ( $i=0; $i<count($obuf); $i++ ) {
				echo "Octeto[$i] =". ord($obuf[$i])."<br>";
			}
		}
		...
		if ( $OctetRecu[7] != $obuf[7] ) { // Lecture du 8eme octet = Code function renvoy?ar destinataire
			echo "<FONT SIZE='3' COLOR='#090762'><b>";
			echo "Error de lectura de los bits de entradas de ".sprintf("1%05d", $this->DebutAdresse)." ".sprintf("1%05d", ($this->DebutAdresse + $this->Nbre))."<br>"; 
			echo "</b></FONT>\n";
			$this->MemoConn = False;
			return array();
		}
	...
	function ReadCoils() {
		...
		if ( $this->Debug ) { //Affichage des octets emis si en mode Debug
			echo "<b>Lectura de las bobinas</b><br>"; 
			for ( $i=0; $i<count($obuf); $i++ ) {
				echo "Octeto[$i] =". ord($obuf[$i])."<br>";
			}
		}
		...
		if ( $OctetRecu[7] != $obuf[7] ) {
			echo "<FONT SIZE='3' COLOR='#090762'><b>";
			echo "Error de lectura de los bits de salidas de ".sprintf("%06d", $this->DebutAdresse)." ".sprintf("%06d", ($this->DebutAdresse + $this->Nbre))."<br>\n"; 
			echo "</b></FONT>\n";
			$this->MemoConn = False;
			return array();
		}
	...
	function WriteHoldRegisters() {
		...
		if ( $this->Debug ) { 
			echo "<b>Registros escritos</b><br>"; 
			for ($i=0;$i<count($obuf);$i++ ) {
				echo "Octeto[$i] =". ord($obuf[$i])."<br>";
			}
		}		
		...
		if ( $OctetRecu[7] != $obuf[7] ) { 
			echo "<FONT SIZE='3' COLOR='#090762'><b>Error de escritura</b></FONT><br>\n";
			$this->MemoConn = False;
			return False;
		}
	...
	function WriteCoil() {
		...
		if ( $OctetRecu[7] != $obuf[7] ) { 
			echo "<FONT SIZE='3' COLOR='#090762'><b>Error de escritura de la bobina ".sprintf("%06d", $Adresse)."</b></FONT><br>\n";
			$this->MemoConn = False;
			return False;
		}
	...
	function SetBridgeRoute() {
		...
		if ( $OctetRecu[7] != $obuf1[7] ) { //8eme octet = Code function renvoye par destinataire
			echo "<FONT SIZE='3' COLOR='#090762'><b>";
			echo "Error de escritura enla función de enrutado dinamico de la pasarela (SetBridgeRoute) en la IP ".$this->AdIpPLC."<br>\n";	
			echo "</b></FONT>\n";
			$this->MemoConn = False;
			fclose($this->Fp);
...
class ModbusSingle extends ModbusTcp {
	...
	function ReadReg( $Adr = "400001" ) {
		...
		if ( $OctetRecu[7] != $obuf[7] ) { // Lecture du 8eme octet = Code function renvoye par destinataire
			echo "<FONT SIZE='3' COLOR='#090762'><b>";
			echo "Error de lectura del registro<br>";
			echo "</b></FONT>\n";
			$this->MemoConn = False;
			return array();
		}
